...
DEFAULT_COSTS: Dict[str, ModelCosts] = {
    ...
    "gemini-2.5-pro-preview-03-25": ModelCosts(
        cost_per_million_input_tokens=1.25,
        cost_per_million_output_tokens=10.0,
        cost_per_million_caching_input_tokens=0.3125,
        cost_per_million_caching_hit_tokens=0.0,
...
def get_model_costs(
    model_name: str, custom_costs: Optional[Dict[str, ModelCosts]] = None
) -> ModelCosts:
    ...
    for key in costs_map:
        if key.startswith(model_name):
            log.warning(
                f"Exact match for '{model_name}' not found, using costs for '{key}'"
            )
...
def calculate_cost(
    model_name: str, result: Any, custom_costs: Optional[Dict[str, ModelCosts]] = None
) -> float:
